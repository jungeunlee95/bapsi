...
<body>
   ...
   <section class="features12 cid-r5sLXhqF4t" id="features12-2p">
      ...
      <div class="container">
         ...
         <c:when test="${empty recipe.file_fakename}">
            <img
               ...
               style="width:600px; height: 400px; " alt="이미지를 불러올수 없습니다.">
         </c:when>
         <c:otherwise>
            <img
      ...
      <div class="media-container-row" id="user_info2">
      <!-- 프로필사진 -->
            <c:choose>
               <c:when test="${empty writerPhoto}"><br>
                  <img
                     src="${pageContext.request.contextPath}/resources/images/Bapsi_logo.png"
                     width="300px" height="300px" alt="이미지를 불러올수 없습니다." 
                     style="width: 170px; height:150px; background-color:white;
                      border: 2px solid #998686; border-radius: 100px;
         ...
         <div class="media-container-row">
            <br>
            <a href="${pageContext.request.contextPath}/ranking/userPage?no=${recipe.userNo}&userNo=${userVO.no}">
            <h4 style="text-align: center; padding-top: 20px;" id="userNickName"><strong>${recipe.userNickName}</strong></h4>
            </a>
            <button id="follow"
               class="<c:out value="${followCheck == 0? 'btn btn-primary-outline display-4':'btn btn-primary-outline display-4 active'}"/>"
               style="width: 150px;heignt:20; font-size: 13px; cursor:pointer;">
                  ...
                  <div class="card-box">
                     <c:forEach items="${ tagList }" var="tag" varStatus="loop">
                        <a
                           ...
                           <p style="text-align: center; color: blue;" id="userNickName"
                              class="mbr-text mbr-section-text mbr-fonts-style display-7">
                              # ${tag.tagName }</p>
                        </a>
                     </c:forEach>
   ...
   <section class="mbr-section content8 cid-r5sQEYUmOJ" id="content8-2t">
      ...
      <div class="container">
         ...
         <div>
               <table class="table">
                  <tr>
                     <td>
                        ...
                        <div>
                           <button id="submitBtn" class="btn pull-right btn-success" style="cursor:pointer;">댓글
                              작성</button>
                        </div> <!-- 대화 수정 창 -->
            ...
            <div id="modifyDiv" style="display: none;">
               ...
               <div align="center">
                  <button type="button" id="reModifyBtn" style="cursor:pointer;">수정</button>
                  <button type="button" id="closeBtn" style="cursor:pointer;">닫기</button>
               </div>
            </div>
      ...
      $("#reply").on("click", ".reList button", function() {
         var li = $(this).parent(); //바보

         var no = li.attr("data-no");
//          var reContent = document.getElementById('replyE');
      var reContent = $(this).parent().children('.replyE').text();
      ...
      // 삭제 버튼 - ul안의 click reList의 버튼 이벤트 설정 function 
      $("#reply").on("click", ".reList .deleteC", function() {
         if(confirm("삭제하시겠습니까?")){
         var li = $(this).parent();
         var no = li.attr("data-no");
   ...
   $(document).ready(function() {
      ...
      <div class="media-container-row">
         <c:if test="${ recipe.userNo eq userVO.no }">
            <button type="button" class="btn  btn-default btn-sm" style="align-content: center;"
               onclick="location.href='${pageContext.request.contextPath}/recipe/update?no=${recipe.no}&page=${fCri.page}&numPerPage=${fCri.numPerPage}&findType=${fCri.findType }&keyword=${fCri.keyword }'" style="cursor:pointer;">수정</button>
            &nbsp;&nbsp;
            <button type="button" class="btn  btn-default btn-sm"
               onclick="location.href='${pageContext.request.contextPath}/recipe/delPage?no=${recipe.no}&page=${fCri.page}&numPerPage=${fCri.numPerPage}&findType=${fCri.findType }&keyword=${fCri.keyword }'" style="cursor:pointer;">삭제</button>
            &nbsp;&nbsp;
            </c:if>
         
            <button type="button" class="btn  btn-default btn-sm"
               onclick="location.href='${pageContext.request.contextPath}/recipe/recipeList?page=${fCri.page}&numPerPage=${fCri.numPerPage}&findType=${fCri.findType }&keyword=${fCri.keyword }'" style="cursor:pointer;">목록</button>
            &nbsp;&nbsp;<br> <br> <br> <br>
        </div>
         <hr>
      </div>
